...
# # app.add_middleware(
# #     CORSMiddleware,
# #     allow_origins=["http://localhost:5173","https://qa-chatbot-seven.vercel.app"],
...
# from langchain_text_splitters import RecursiveCharacterTextSplitter
# from langchain_community.vectorstores import FAISS
# from langchain_community.embeddings import FastEmbedEmbeddings  # lightweight ONNX embeddings (no torch)
...
# def get_embeddings():
#     """
#     Lightweight embedding model using fastembed (ONNX runtime).
#     This avoids pulling in torch/sentence-transformers, which is what
#     was causing the 'Out of memory (used over 512Mi)' crash on Render.
#     """
#     global embeddings

#     if embeddings is None:
#         print("Loading lightweight embedding model (fastembed)...")
...
def preload_models():
    """
    Preload the embedding model and any existing FAISS index at server boot,
    instead of on the first user request. This avoids the first upload/chat
    request timing out on the frontend while fastembed downloads its ONNX
    model files (~100-200MB) for the first time.
    """
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"Startup: failed to preload embeddings: {e}")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        print(f"Startup: failed to load existing vectorstore: {e}")
