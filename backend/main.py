...
# HuggingFace Inference API key (free account token)
...
def get_embeddings():
    ...
    No model is loaded locally — embeddings are computed on HF's servers
    via HTTP calls. This keeps local RAM usage near zero and prevents
    the 512MB OOM crash on Render's free tier.
    """
    global embeddings
    ...
    if embeddings is None:
        ...
        print("HF Inference API embeddings ready.")
...
async def upload_pdf(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        return {"message": "PDF processed", "chunks_added": len(documents)}
...
def upload_youtube(request: YouTubeRequest):
    ...
    return {"message": "YouTube processed", "chunks_added": len(all_documents)}
